---
layout: post
title:  Caffe, 多维数组的行优先和列优先, 推荐系统的工程细节
category: AI 
---
...
>注：在API层面，Caffe和OpenVX顺序完全相反，但是由于OpenVX是列优先存储，因此在内存存储方面，两者的格式正好完全一致。详见下面的“多维数组的行优先和列优先”一节。
...
1.对数组、复数等数值计算的原生支持。语法糖，对于非程序员的科技人员很友好。
