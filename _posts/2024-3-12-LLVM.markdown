...
ModuleOp::lookupSymbol底层的数据结构是链表，因此这个操作的复杂度是$$O(n)$$，如果再对每个元素执行这个操作，复杂度就是$$O(n^2)$$。

这是可以用DenseMap，它底层数据结构是开放寻址哈希表（Open Addressing Hash Table），采用二次探测（Quadratic Probing）解决冲突。

与之相比`std::unordered_map`虽然也是哈希表，但是使用链地址法（Separate Chaining）解决冲突。
