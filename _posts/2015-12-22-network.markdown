...
一个事件包含若干参数，因此事件的数据结构是一个argument数组。以SetVolume事件为例，它的参数结构的实例为src/upnp_control.c: arguments_set_vol。

4)服务

一个服务包含若干事件，因此服务的数据结构是一个argument的二维数组。在这里是src/upnp_control.c: argument_list。

2.事件发布

事件发布用于向外界宣布本服务所支持的功能。gmediarender事件发布的内容是动态生成的，其函数为：src/upnp.c: gen_scpd
