...
NCCL 2.23版本引入了一种新的集合算法——PAT（Parallel Aggregated Trees，并行聚合树）。这是一种典型的Ring和Tree混合策略。
...
NVSHMEM是NVIDIA开发的一种基于OpenSHMEM标准的并行编程库，专为NVIDIA GPU和NVLink/NVSwitch高速互联架构设计。

OpenSHMEM的通信接口按照功能来分主要有如下的几个部分：

- 远程内存直接访问：SHMEM_PUT、SHMEM_GET等（阻塞式）、SHMEM_PUT_NBI、SHMEM_GET_NBI等（非阻塞式）
- 远程内存原子操作：SHMEM_ATOMIC_ADD、SHMEM_ATOMIC_SWAP等，及其非阻塞版
- 聚合通信操作：SHMEM_BARRIER、SHMEM_ALLTOALL等

http://blog.sysu.tech/Research/OpenSHMEM%E7%9A%84%E4%B8%80%E7%82%B9%E8%B0%83%E7%A0%94/
