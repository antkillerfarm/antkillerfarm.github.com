---
layout: post
title:  小众语言集中营, Julia, Go, Lua, Kotlin, Github显示数学公式, VS
category: language 
---
...
TikZ是一个用于绘图的Tex包。TikZ基于PGF（portable graphics format）。TikZ ist kein Zeichenprogram(TikZ is not a drawing program)。

安装：

`sudo apt install qtikz texlive-fonts-recommended texlive-latex-extra`
