...
你不用管消息是如何产生的，你只要创建消息处理函数。等消息出现了，自然就会调用你的消息处理函数。

而许多程序员心心所念的main函数，则变成了框架的一部分。

时至今日，写安卓程序也是这样，第一个函数就是onCreate，也就是窗口创建回调。至于窗口创建代码，也是写在安卓框架里面的。

但是，当时大家接受不了啊！！！没有main函数，一个C语言程序怎么能执行呢！！！
...
Robert W. Floyd，1936–2001，美国计算机科学家，1978年图灵奖得主。Floyd没有博士学位。他本科读的是文理学院（芝加哥大学），学的是人文学科，后来自学计算机，靠论文和研究成果直接进入了学术界，最终成为斯坦福大学教授。这在今天几乎不可想象。

Floyd-Warshall算法​：求所有点对之间最短路径（动态规划经典）
...
如果跑道是直的（无环），兔子会永远跑在乌龟前面，永远不会相遇。如果跑道有环，兔子最终会从后面追上乌龟——两者一定会相遇。

反转链表怎么“判断”环？如果链表无环，你可以把它从头到尾完整反转；如果链表有环，反转过程会“走不出去”，或者结构会出问题。缺点非常明显：有环时程序可能永远不结束。
