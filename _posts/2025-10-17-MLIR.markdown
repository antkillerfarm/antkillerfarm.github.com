...
- Transforms文件夹做“内部优化/规范化”。所以一般是包含在某个Dialect名下的。
- Conversion文件夹做“Dialect到Dialect的逐步lowering”；
- Target文件夹做“最后一跳”，生成真正能让外部编译器/硬件直接消费的比特流或文本。Conversion和Target一般放在和Dialect同级的文件夹下。
...
MegaKernel有时也被称为Persistent Kernel。

---

CUDA Graph的核心机制：Capture → Instantiate → Launch

Graph要求虚拟地址在capture和replay之间保持不变，导致PyTorch只能为每个Graph隔离独立的memory pool，内存常驻不可复用。
...
https://zhuanlan.zhihu.com/p/2031867129746679499

如何解决 CUDA Graph 的静态显存开销（上）