...
ubuntu通常会在LTS版本发布3～6个月之后才会将新版本放到release通道，在此之前需要通过如下方式升级：
...
Ubuntu为了降低风险，把部分软件包的新版本做成了“分阶段发布（phased update）”。

简单来说，这些更新会先只推送给一小部分用户做“灰度测试”；如果几天内没出现明显Bug，再逐步扩大推送范围，直到100%用户都能收到。
