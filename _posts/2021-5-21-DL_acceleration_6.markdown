...
矩阵外积乘法的Swamping效应，是低维向量外积生成高维秩1矩阵时，数值范围急剧膨胀、大数吃掉小数，导致浮点精度灾难性损失、小信号被舍入误差完全掩盖的数值不稳定现象。

FP16/BF16矩阵乘内积下，累加做到38bit-42bit就基本满足精度要求了。但是外积需要69bit的累加精度，才能做到和内积相同的精度！

---

使用内积乘法的硬件：NV的tensor core、Intel的AMX。

使用外积乘法的硬件：ARM的SME、Intel/AMD的ACE。

Intel为什么要放弃AMX，使用ACE？而NV一直坚持内积？

GPU每个SM自带大容量片上共享内存，张量核心内积可以直接依托共享内存做矩阵缓存，不需要新增全局专用寄存器，内积硬件的面积、调度劣势在GPU上完全消失，反而能最大化流水线吞吐。
