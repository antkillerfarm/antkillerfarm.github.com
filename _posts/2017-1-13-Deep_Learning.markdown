...
上图是一个Web图模型的示例。其中的节点表示网页，箭头表示网页链接。因此，从图论的角度来说，这是一个有向图。
...
马尔可夫链的基本定义参见《机器学习（十六）》。

这里补充一些定义：

**定义1**：设C为状态空间的一个子集，如果从C内任一状态i不能到C外的任何状态，则称C为**闭集**。除了整个状态空间之外，没有别的闭集的马尔可夫链被称为**不可约的**。

如果用状态转移图表示马尔可夫链的话，上面的定义表征了马尔可夫链的**连通性**。
...
推论3. 若$$X_j$$是不可约的非周期的马尔可夫链的平稳分布，则$$\lim_{n\to\infty}P_{ij}^{(n)}=X_j$$，即极限分布等于平稳分布。
