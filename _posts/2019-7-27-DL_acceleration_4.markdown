...
CloseAI带了个坏头，取消了输出中的概率分布。只给前五个词。到了这一步，其实传统的蒸馏意义就不大了。

后来人们用的更多是利用高级模型语义的正确性和敏感性，生成一大堆训练数据，再由大模型对这些数据进行不断的筛选、反思和打分。然后用这些品质更高的数据，对小模型进行微调。

现在的蒸馏更多是利用思维链的输出，去模拟大模型如何得到更优秀答案的思考过程。到这一份上，你可以说，这和“传统的蒸馏”已经毫无关系了。

glm 5.2这么强，社区普遍认为用的是anyrouter中转站的数据，因为背后的公司和智谱关系密切。
