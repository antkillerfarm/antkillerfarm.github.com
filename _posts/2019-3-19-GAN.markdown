...
=&\mathop{\arg\min}_{\Theta} \frac{1}{B}\sum_{i=1}^B\left[D\Big(z_i,\Theta\Big)-D\Big(y_i,\Theta\Big)\right]\end{aligned}\tag{1}$$
...
=&\mathop{\arg\min}_{\theta} \frac{1}{B}\sum_{i=1}^B\left[D\Big(G(x_i,\theta),\Theta\Big)\right]\end{aligned}\tag{2}$$
...
4.用fake_data和Real Samples共同训练D网络，分别可得到$$D_{fake}$$和$$D_{real}$$。使用公式1计算总Loss：D。

5.固定D网络参数，使用公式2训练网络D(G)中G的参数。

6.相对于监督学习，生成模型G的参数更新不是来自于数据样本本身（不是对数据的似然性进行优化），而是来自于判别模型D的一个反传梯度。因此我们常常需要D更新K次，G才能更新1次。
