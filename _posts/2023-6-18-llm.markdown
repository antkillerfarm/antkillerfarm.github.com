...
OPD（On-Policy Distillation）：在蒸馏过程中，学生模型（Student）基于自己当前的策略实时生成数据（On-Policy Sampling），然后由教师模型（Teacher，通常是经过 RL 阶段训练的强大模型）对这些生成结果提供监督信号（如 logits、隐藏状态或奖励），学生模型据此更新参数，使自己的策略逐步逼近教师模型。避免了“训练时看教师的正确答案，推理时却要基于自己之前可能出错的token继续生成”的曝光偏差（Exposure Bias）问题。
