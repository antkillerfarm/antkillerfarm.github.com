...
Representation Network的主要思想来自如下论文：
...
这里用$$\vec{s}$$表示系统的抽象状态以区别其实际状态s。也就是说，在系统模型中，预测的不是实际的状态s, 而是抽象的状态。
...
这里的套路其实和DQN非常像，区别在于：这里既然是Model-based方法，那么自然有利用Model生成模拟样本的步骤，而DQN没有这样的步骤。

如果说参考论文1还有点DQN硬往Model-based上靠的话，参考论文2的方法就优雅的多了。

![](/images/img3/MuZero_4.png)

$$\mathbf{Encoding}\quad f_\theta^{enc}$$
