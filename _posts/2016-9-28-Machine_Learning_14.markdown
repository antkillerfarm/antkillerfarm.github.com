...
一个n阶矩阵有n个特征值，这些特征值可按绝对值大小排序，绝对值越大的，越重要。其中最大的k个特征值，被称作k principal components，这就是主成分分析（Principal components analysis，PCA）算法的命名来历。可以看出PCA算法实际上是个**降维（dimensionality reduction）**算法。
...
>注：严格的说，在这个例子中，由于矩阵不是方阵，所以这里取的是奇异值，但特征值也有类似的结论。
