...
值得注意的是该项目是从2007年开始的，且并非基于EA在2020年开源的那份代码。而是完全的引擎重写。
...
github.com/electronicarts/CnC_Generals_Zero_Hour

《命令与征服：将军》及其资料片《零点行动》的源代码

至于《红警2》的源代码（即所谓的TS引擎）貌似在EA收购中下落不明，永远消失了，所以也就没有高清重制版了。

TS引擎非常复杂，OpenRA没法覆盖TS引擎的全部内容，大概还有80%的功能缺失。

Syringe是红警2modding社区开发的一个DLL Injector，它在游戏启动时，将自定义的DLL文件注入到游戏进程gamemd.exe的内存空间中。这类似于给运行中的游戏做"动态手术"——不需要修改原始游戏文件，就能在内存层面替换或扩展游戏功能。

Ares和Phobos引擎都使用了Syringe的技术。

反向走私确实存在。暴雪决定开怀旧服时，尴尬的发现，60级的代码，全公司已经没有了，最后居然是从私服搞的魔改后的代码，再逆向工程修改成接近原版的效果。
...
https://zhuanlan.zhihu.com/p/1959574914458711924

没有源代码，他用5年时间把红警2给重写了

https://zhuanlan.zhihu.com/p/1900333141173970128
...
金庸群侠传 3D复刻版（正式名：群侠传，启动，这是一个游戏引擎，进入后可以从创意工坊下载不同MOD，每个MOD就是一个游戏，比较好的自制模组《诗酒芳华录》，属于后宫版群侠传，今天还在一直更新，质量不输苍龙。）
