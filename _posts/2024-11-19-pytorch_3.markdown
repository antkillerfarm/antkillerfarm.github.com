...
PyTorch有Structured Kernel和Unstructured Kernel两种不同的算子实现方式。

Structured Kernel：框架自动推断并创建输出张量，自动生成形状检查、设备检查等。能显著减少算子开发工作量并保证行为一致性。

Unstructured Kernel：手工编写所有过程，具有最高的灵活性，适合复杂场景。

Structured Kernel的实现包含`meta`和`impl`两个部分。前者负责输出张量的形状、dtype、设备推断等，但不执行实际的数据计算。后者才是执行数据计算的地方。
...
$$| actual−expected | \leq atol + rtol * | expected |$$

---

普通张量要求所有元素形状一致（矩形），而NestedTensor允许"内部维度长度不一"（锯齿状/Jagged），不用填充让变长数据在GPU上高效处理，是训练Transformer等模型时的重要优化手段。

NestedTensor使用偏移量索引，记录每个序列从哪里开始。
